...
test('Stage25AE updated special ability values are authoritative',()=>{
  ...
  const cover=getAbility('Archer','COVER_FIRE').basicStyle;assert.equal(cover.attacksSet,4);assert.equal(getAbility('Archer','VOLLEY').completionDelayCycles,4);assert.equal(getAbility('Archer','SNIPE').basicStyle.damageMultiplier,2.10);
