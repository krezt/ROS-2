...
const classes = {
 ...
 Rogue:{baseStats:{maxHP:1300,QKN:30},identity:'High-pressure precision melee attacker; crits, poison imbues, marks and evasive disruption.',combat:{movementMax:15,attacksMax:9,attackInterval:1},weapon:{weaponProfileId:'ROGUE_BLADE',mode:'MELEE',weaponRange:1,preferredRange:1,counterMoveMax:1,attackBaseMin:55,attackBaseMax:85,critBonus:.10},abilities:[
 ...
 Cleric:{baseStats:{maxHP:1384,QKN:15},identity:'Primary restorative and defensive support with healing, poison cleansing, protection and resurrection.',combat:{movementMax:11,attacksMax:6,attackInterval:2},weapon:{weaponProfileId:'CLERIC_MACE',mode:'MELEE',weaponRange:2,preferredRange:2,counterMoveMax:1,attackBaseMin:35,attackBaseMax:60},abilities:[
  ab('CLERIC_ATTACK','Attack',0,A.BASIC_ATTACK,T.UNIT,[],{basicProc:{type:'HEAL_SELF',roundChance:.80,referenceSwings:6,maxPerRound:3,min:10,max:16,scalesWith:'SDM',label:'Prayer Mend'}}),
  ab('DEFENSIVE_AURA','Defensive Aura',2,A.SPELL,T.SELF,[{type:'HEAL_PERCENT_ROLL',minPct:.30,maxPct:.40,to:'SELF'},status('def_up',4,{to:'SELF'})],{note:'2-cycle self-only focus-fire counterplay: restore a randomized 30–40% max HP and substantially reinforce DEF.'}),
  ab('PIERCING_LIGHT','Piercing Light',3,A.SPELL,T.GROUND,[{type:'AOE_DAMAGE',min:150,max:230,damageType:D.MAGICAL,scalesWith:'SDM',defensePenetration:.30,dodgeable:false,hostileOnly:true},{type:'AOE_REMOVE_STATUS',key:'invisible',hostileOnly:true,reason:'PIERCING_LIGHT_REVEAL'}],{area:{shape:'SQUARE_4X4'},castRange:999,note:'3-cycle 4x4 holy AoE: damages opposing occupants and strips Invisibility from opposing champions caught in the area.'}),
  ...
  ab('RESURRECTION','Resurrection','slow',A.SPELL,T.UNIT,[{type:'RESURRECT_OR_HEAL',revivePctMaxHP:.55,livingHealPctMaxHP:.25,cleanse:true}],{usesMax:2,usesKey:'resurrection',allowDeadTarget:true}),
 ...
 Mage:{baseStats:{maxHP:1300,QKN:14},identity:'High-impact ranged magical damage and team magical protection; AoE, amplification and spell duplication.',combat:{movementMax:11,attacksMax:6,attackInterval:2},weapon:{weaponProfileId:'MAGE_DAGGER',mode:'MELEE',weaponRange:3,preferredRange:3,counterMoveMax:1,attackBaseMin:25,attackBaseMax:45},abilities:[
 ...
 Paladin:{baseStats:{maxHP:1427,QKN:12},identity:'Defensive melee/support hybrid; protection, cleanse, stun and holy payoff damage.',combat:{movementMax:13,attacksMax:6,attackInterval:2},weapon:{weaponProfileId:'PALADIN_SWORD',mode:'MELEE',weaponRange:2,preferredRange:2,counterMoveMax:1,attackBaseMin:70,attackBaseMax:105},abilities:[
  ab('PALADIN_ATTACK','Attack',0,A.BASIC_ATTACK,T.UNIT,[],{basicProc:{type:'STATUS_SELF',roundChance:.75,referenceSwings:6,maxPerRound:3,key:'def_up',duration:2,label:'Resolve'}}),
  ...
  ab('JUDGMENT','Judgment','slow',A.SPELL,T.UNIT,[{type:'CONDITIONAL_DAMAGE',min:180,max:260,scalesWith:'SDM',damageType:D.MAGICAL,defensePenetration:.25,afflictedMultiplier:1.6}])
 ]},
 Archer:{baseStats:{maxHP:1300,QKN:14},identity:'Long-range physical pressure; precision, marks, ranged control and area barrages.',combat:{movementMax:12,attacksMax:7,attackInterval:2},weapon:{weaponProfileId:'LONGBOW',mode:'RANGED',weaponRange:6,preferredRange:6,counterMoveMax:3,attackBaseMin:55,attackBaseMax:85,critBonus:.08},abilities:[
  ab('ARCHER_ATTACK','Attack',0,A.BASIC_ATTACK,T.UNIT,[]),
  ab('RANGERS_FOCUS',"Ranger's Focus",'fast',A.SPELL,T.UNIT,[heal(50,100,{scalesWith:'ATK'}),status('atk_up',4,{stackMode:'STACK'})],{note:'1-cycle support spell: heal 50–100 (scaled by ATK) and grant ATK Up for 4 rounds.'}),
  ...
  ab('SNIPE','Snipe',0,A.BASIC_ATTACK,T.UNIT,[],{basicStyle:{attacksSet:3,damageMultiplier:2.50,attackRangeOverride:8,ordinaryKite:true,distanceDamageBonusPerSquare:.10},note:'Three long-range kiting shots at 250% base damage, plus 10% more damage per square of actual separation when each shot is fired.'}),
 ...
 Necromancer:{baseStats:{maxHP:1300,QKN:14},identity:'Attrition caster centered on poison buildup, life stealing, percentage damage and detonation.',combat:{movementMax:11,attacksMax:5,attackInterval:2},weapon:{weaponProfileId:'NECRO_DAGGER',mode:'MELEE',weaponRange:3,preferredRange:3,counterMoveMax:1,attackBaseMin:15,attackBaseMax:30},abilities:[
  ab('NECRO_ATTACK','Attack',0,A.BASIC_ATTACK,T.UNIT,[],{basicProc:{type:'LIFE_DRAIN',roundChance:.80,referenceSwings:5,maxPerRound:3,min:28,max:40,scalesWith:'SDM',damageType:D.MAGICAL,label:'Life Drip'}}),
  ab('LIFE_DRAIN','Life Drain','fast',A.SPELL,T.UNIT,[{type:'LIFE_DRAIN',min:150,max:200,scalesWith:'SDM'}],{note:'Deals 150–200 base magical damage (scaled by SDM) and heals the Necromancer for exactly the damage actually dealt.'}),
  ab('DEATH_TOUCH','Death Touch','slow',A.SPELL,T.UNIT,[{type:'CURRENT_HP_DAMAGE',fraction:.50}]),
  ab('POISON_BOLT','Poison Bolt','normal',A.SPELL,T.UNIT,[dmg(120,190,{damageType:D.MAGICAL,scalesWith:'SDM',critBonus:.20,dodgeable:false,tag:'POISON_BOLT'}),{type:'STRIP_BENEFICIAL',count:1},{type:'POISON_FROM_LAST_DAMAGE',ratio:.70}],{note:'Deals 120–190 base magical damage, strips one beneficial status, then adds Poison equal to 70% of damage dealt.'}),
 ...
 Mystic:{baseStats:{maxHP:1171,QKN:27},identity:'Control/disruption specialist with stun, berserk, defense collapse, scheduler manipulation and throwing-dagger backup.',combat:{movementMax:13,attacksMax:10,attackInterval:1},weapon:{weaponProfileId:'THROWING_DAGGER',mode:'RANGED',weaponRange:999,preferredRange:999,counterMoveMax:2,attackBaseMin:35,attackBaseMax:55,behavior:WEAPON_BEHAVIOR.THROWING_DAGGER,retargetPolicy:RETARGET_POLICY.IN_RANGE_RANDOM},abilities:[
  ...
  ab('MENTAL_BREAKDOWN','Mental Breakdown',2,A.SPELL,T.UNIT,[status('spellbreak',3,{chance:.90,data:{announceResist:true}})],{note:"2-cycle anti-caster disruption: 90% chance to apply Spellbreak. It cancels the target's next spell attempt; against an Arcane Echo spell it cancels only the first resolution, allowing the echoed second resolution to continue."}),
  ab('MIND_SHATTER','Fear',3,A.SPELL,T.UNIT,[status('suppression',2)],{note:'3-cycle counter-control spell: Suppresses the target for two rounds, preventing counterattacks without stopping proactive action.'}),
 ...
 Shinobi:{baseStats:{maxHP:1258,QKN:24},identity:'Tight-range dexterity melee assassin; extreme dagger volume, invisibility, pursuit mobility, bleed utility and disruption.',combat:{movementMax:16,attacksMax:12,attackInterval:1},weapon:{weaponProfileId:'SHINOBI_DAGGER',mode:'MELEE',weaponRange:1,preferredRange:1,counterMoveMax:1,attackBaseMin:45,attackBaseMax:75,critBonus:.30,retargetPolicy:RETARGET_POLICY.NEAREST_IN_RANGE},abilities:[
  ...
  ab('REGEN_POTION','Regen Potion','fast',A.ITEM,T.SELF,[{type:'HEAL_PERCENT_ROLL',minPct:.30,maxPct:.40,to:'SELF'},{type:'CLEANSE',scope:'SELF',keys:['poison']}],{usesMax:4,usesKey:'potion_regen',note:'Heal 30–40% max HP and cure Poison.'}),
 ...
 Electromancer:{baseStats:{maxHP:1214,QKN:15},identity:'Aggressive magical hybrid; electricity, stun, chain damage, self-reset, evasion and team offense.',combat:{movementMax:12,attacksMax:7,attackInterval:1},weapon:{weaponProfileId:'SHOCK_GAUNTLET',mode:'MELEE',weaponRange:2,preferredRange:2,counterMoveMax:1,attackBaseMin:18,attackBaseMax:28},abilities:[
  ...
  ab('ELECTRICAL_STORM','Electrical Storm','fast',A.SPELL,T.ALL_ENEMIES,[{type:'HYBRID_STORM',damage:{min:25,max:40,},heal:{min:10,max:20,},stunChance:.15}],{migrationStatus:ROSTER_MIGRATION_STATUS.PROVISIONAL,spatialPolicy:'LEGACY_GLOBAL_PENDING_REVIEW'}),
  ...
  ab('GOD_TEMPEST','God Tempest','normal',A.SPELL,T.SELF,[{type:'CLEANSE',scope:'SELF',mode:'NEGATIVE'},{type:'FULL_HEAL',to:'SELF'},status('sdm_up',6,{to:'SELF',stackMode:'STACK'}),status('def_up',6,{to:'SELF',stackMode:'STACK'})],{usesMax:1,usesKey:'god_tempest'}),
