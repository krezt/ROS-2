...
const classes = {
 Warrior:{baseStats:{maxHP:2126,QKN:17},identity:'Durable frontline melee fighter; protection, taunt, sustain and strong single-target pressure.',combat:{movementMax:14,attacksMax:7,attackInterval:2},weapon:{weaponProfileId:'SWORD_SHIELD',mode:'MELEE',weaponRange:2,preferredRange:2,counterMoveMax:1,attackBaseMin:80,attackBaseMax:120},abilities:[
  ...
  ab('DIG_IN','Dig In','fast',A.ABILITY,T.SELF,[status('def_up',2,{to:'SELF'}),status('physical_shield',2,{to:'SELF',data:{pct:.20}}),heal(0,0,{to:'SELF',pctMaxHP:.20})],{note:'Focus-fire deterrent: restore 20% max HP while digging in behind DEF and a 20% Physical Shield.'})
 ]},
 Barbarian:{baseStats:{maxHP:1997,QKN:16},identity:'Aggressive heavy melee bruiser; reach, burst, defense break, stun and relentless pursuit.',combat:{movementMax:15,attacksMax:7,attackInterval:2},weapon:{weaponProfileId:'LONG_AXE',mode:'MELEE',weaponRange:3,preferredRange:3,counterMoveMax:1,attackBaseMin:77,attackBaseMax:108},abilities:[
  ...
  ab('SMASH','Smashing Blows',0,A.BASIC_ATTACK,T.UNIT,[],{basicStyle:{attacksDelta:-2,damageMultiplier:1.10,onHit:{statusKey:'stun',chance:.15,duration:2}},note:'Alternative attack style: -2 attacks, 110% damage, 15% 2-round Stun per successful hit.'}),
  ab('REND','Rend',0,A.BASIC_ATTACK,T.UNIT,[],{basicStyle:{attacksSet:4,onHit:{defenseShredPct:.25,statusKey:'rend_def_down',duration:3}},note:'Alternative attack style: exactly 4 swings; each successful hit stacks a multiplicative 25% DEF reduction for 3 rounds.'}),
 ...
 Rogue:{baseStats:{maxHP:1700,QKN:30},identity:'High-pressure precision melee attacker; crits, poison imbues, marks and evasive disruption.',combat:{movementMax:17,attacksMax:9,attackInterval:1},weapon:{weaponProfileId:'ROGUE_BLADE',mode:'MELEE',weaponRange:1,preferredRange:1,counterMoveMax:1,attackBaseMin:55,attackBaseMax:100,critBonus:.15},abilities:[
  ...
  ab('SHADOWSTEP','Shadowstep','fast',A.SPELL,T.SELF,[status('invisible',3,{to:'SELF',data:{breakOnPhysicalAttack:true,sourceAbility:'SHADOWSTEP'}}),status('shadowstep_crit',3,{to:'SELF',data:{multiplier:2.5}})],{note:'Three-round stealth setup. Shadowstep Invisibility breaks on the Rogue\'s first physical attack; critical hits use a 2.5× multiplier while the Shadowstep crit window remains active, long enough to chain Shadowstep → Expose → Backstab.'}),
 ...
 Paladin:{baseStats:{maxHP:1827,QKN:12},identity:'Defensive melee/support hybrid; protection, cleanse, stun and holy payoff damage.',combat:{movementMax:15,attacksMax:7,attackInterval:2},weapon:{weaponProfileId:'PALADIN_SWORD',mode:'MELEE',weaponRange:2,preferredRange:2,counterMoveMax:1,attackBaseMin:70,attackBaseMax:105},abilities:[
  ab('PALADIN_ATTACK','Attack',0,A.BASIC_ATTACK,T.UNIT,[],{basicProc:{type:'STATUS_SELF',chance:.25,key:'def_up',duration:2,label:'Resolve'}}),
  ab('SHIELD_BASH','Shield Bash',0,A.BASIC_ATTACK,T.UNIT,[],{basicStyle:{attacksSet:3,ordinaryAttackLimit:1,startupDelayCycles:1,damageMultiplier:3.5,onHit:{statusKey:'stun',chance:.35,duration:2},selfOnFirstAttack:{statusKey:'physical_shield',duration:1,data:{pct:.20}}},note:'Pursuit bash. After a 1-cycle startup, Paladin pursues normally and makes one 350% weapon attack; two remaining attack resources are reserved for normal counters. A landed bash has a 35% chance to Stun for 2 rounds, then Paladin braces behind a 20% physical shield for the rest of the round.'}),
 ...
 Archer:{baseStats:{maxHP:1700,QKN:14},identity:'Long-range physical pressure; precision, marks, ranged control and area barrages.',combat:{movementMax:12,attacksMax:7,attackInterval:2},weapon:{weaponProfileId:'LONGBOW',mode:'RANGED',weaponRange:6,preferredRange:6,counterMoveMax:3,attackBaseMin:69,attackBaseMax:106,critBonus:.075},abilities:[
  ...
  ab('SNIPE','Snipe',0,A.BASIC_ATTACK,T.UNIT,[],{basicStyle:{attacksSet:3,damageMultiplier:2.2275,attackRangeOverride:8,ordinaryKite:true,distanceDamageBonusPerSquare:.05},note:'Three long-range kiting shots at 222.75% base damage, plus 5% more damage per square of actual separation when each shot is fired. Range 8.'}),
 ...
 Monk:{baseStats:{maxHP:1614,QKN:26},identity:'Fast melee combatant; explosive attack-volume styles, stun pressure, reactive defense and self-recovery.',combat:{movementMax:15,attacksMax:7,attackInterval:1},weapon:{weaponProfileId:'MONK_FISTS',mode:'MELEE',weaponRange:1,preferredRange:1,counterMoveMax:1,attackBaseMin:55,attackBaseMax:65,critBonus:.10},abilities:[
  ...
  ab('SECOND_WIND','Second Wind','normal',A.ABILITY,T.SELF,[{type:'CLEANSE',scope:'SELF',mode:'CC_AND_DEBUFFS'},status('regen',3,{to:'SELF',data:{pct:.15}})],{hardControlBypass:true,note:'May start and resolve while hard-CCed. Regen restores 15% of the Monk’s max HP per round for 3 rounds (242 HP/round at base max HP).'}),
 ]},
 Necromancer:{baseStats:{maxHP:1700,QKN:14},identity:'Attrition caster centered on poison buildup, life stealing, percentage damage and detonation.',combat:{movementMax:12,attacksMax:6,attackInterval:2},weapon:{weaponProfileId:'NECRO_DAGGER',mode:'MELEE',weaponRange:3,preferredRange:3,counterMoveMax:1,attackBaseMin:15,attackBaseMax:30},abilities:[
 ...
 Electromancer:{baseStats:{maxHP:1614,QKN:15},identity:'Aggressive magical hybrid; electricity, stun, chain damage, self-reset, evasion and team offense.',combat:{movementMax:13,attacksMax:8,attackInterval:1},weapon:{weaponProfileId:'SHOCK_GAUNTLET',mode:'MELEE',weaponRange:2,preferredRange:2,counterMoveMax:1,attackBaseMin:18,attackBaseMax:28},abilities:[
  ab('ELECTRO_ATTACK','Attack',0,A.BASIC_ATTACK,T.UNIT,[],{basicProc:{type:'DAMAGE',chance:.30,min:50,max:175,scalesWith:'SDM',damageType:D.MAGICAL,defensePenetration:.10,label:'Lightning Bolt'}}),
