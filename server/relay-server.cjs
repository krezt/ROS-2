...
(async () => {
  ...
  const server = http.createServer((req,res)=>{
    ...
    if(pathname==='/health'){res.setHeader('content-type','application/json; charset=utf-8');res.writeHead(200);res.end(JSON.stringify({ok:true,protocol:'ros2-protocol-1',stage:'25AG',rankedLadder:true,separateFormatRatings:true,championWinRates:true,rankedDraws:true,drawProposals:true,rankedPersistence:rankedPersistence()}));return;}
    ...
    if(pathname==='/'){res.setHeader('content-type','text/plain; charset=utf-8');res.writeHead(200);res.end('ROS 2.0 coordinator OK — Stage 25AG spectator-ready coordinator');return;}
  ...
  function normalizeReplaySpeed(value){const n=Number(value??0.5);if(![0.25,0.33,0.5].includes(n))throw new Error('INVALID_REPLAY_SPEED');return n;}
  ...
  function normalizeRoomConfig(input={}){return Object.freeze({teamSize:normalizeTeamSize(input.teamSize??3),draftBansPerPlayer:normalizeDraftBans(input.draftBansPerPlayer??0),replaySpeed:normalizeReplaySpeed(input.replaySpeed??0.5),ranked:normalizeRanked(input.ranked)});}
  function send(ws,obj){if(ws?.readyState===OPEN)ws.send(JSON.stringify(obj));}
  function roomPeers(room){return [...room.sockets.values(),...room.spectators.values()];}
  function broadcastRoom(room,obj){for(const ws of roomPeers(room))send(ws,obj);}
  ...
  function roomSummary(room){const teamSize=room.config.teamSize,names=playerNames(room),status=phaseOf(room);return{id:room.id,hostName:names.A??'Player',players:room.matchRoom.players.size,maxPlayers:2,spectators:room.spectators.size,spectatable:status==='IN_MATCH',started:!!room.coordinator,configLocked:room.configLocked,teamSize,format:`${teamSize}v${teamSize}`,draftBansPerPlayer:room.config.draftBansPerPlayer,replaySpeed:room.config.replaySpeed,ranked:!!room.config.ranked,draftPhase:room.draft?.phase??null,matchNumber:room.matchNumber,status};}
  ...
  function clearRoomMembership(ws){ws._roomId=null;ws._side=null;ws._role=null;}
  function leave(ws,reason='PLAYER_LEFT'){const room=ws._roomId?rooms.get(ws._roomId):null;if(!room){clearRoomMembership(ws);return;}if(ws._role==='SPECTATOR'){room.spectators.delete(ws._playerId);clearRoomMembership(ws);broadcastRoom(room,{kind:'spectator_presence',roomId:room.id,spectators:room.spectators.size,serverSequence:nextSequence()});broadcastRooms();return;}const departedSide=room.matchRoom.removePlayer(ws._playerId);room.sockets.delete(ws._playerId);clearRoomMembership(ws);if(room.configLocked||departedSide==='A'){const during=phaseOf(room);for(const peer of roomPeers(room)){send(peer,{kind:'opponent_disconnected',roomId:room.id,during,configLocked:true,canRematch:false,serverSequence:nextSequence()});send(peer,{kind:'room_closed',roomId:room.id,reason,serverSequence:nextSequence()});clearRoomMembership(peer);}rooms.delete(room.id);return;}if(room.sockets.size===0)rooms.delete(room.id);}
  function join(ws,room){if(room.configLocked||room.matchRoom.players.size>=2)throw new Error('ROOM_FULL');if(room.config.ranked){ensureRankedStorage();if(!rankedNameAllowed(ws._displayName))throw new Error('RANKED_NAME_REQUIRED');const incoming=normalizePlayerName(ws._displayName).toLowerCase();if(Object.values(playerNames(room)).some(name=>normalizePlayerName(name).toLowerCase()===incoming))throw new Error('RANKED_NAME_IN_USE');}leave(ws,'JOINED_OTHER_ROOM');const side=room.matchRoom.addPlayer(ws._playerId);room.sockets.set(ws._playerId,ws);ws._roomId=room.id;ws._side=side;ws._role='PLAYER';if (room.matchRoom.isReady()) room.configLocked = true;send(ws,{kind:'room_joined',roomId:room.id,side,config:{...room.config},configLocked:room.configLocked,players:room.matchRoom.players.size,playerNames:playerNames(room),serverSequence:nextSequence()});if(room.configLocked){broadcastRoom(room,{kind:'room_locked',roomId:room.id,config:{...room.config},players:2,playerNames:playerNames(room),message:'Lobby configuration locked. Network draft begins.',serverSequence:nextSequence()});beginNetworkDraft(room);}broadcastRooms();}
  function joinSpectator(ws,room){
    if(phaseOf(room)!=='IN_MATCH'||!room.coordinator||!room.matchId||!room.matchTeams)throw new Error('MATCH_NOT_SPECTATABLE');
    leave(ws,'JOINED_OTHER_ROOM');
    room.spectators.set(ws._playerId,ws);ws._roomId=room.id;ws._side=null;ws._role='SPECTATOR';
    const confirmedRoundPackages=room.roundHistory.filter(entry=>entry.confirmed).map(entry=>entry.package);
    const pendingEntry=[...room.roundHistory].reverse().find(entry=>!entry.confirmed&&entry.roundNumber===room.coordinator.roundNumber);
    send(ws,{kind:'spectator_joined',roomId:room.id,role:'SPECTATOR',config:{...room.config},matchId:room.matchId,matchNumber:room.matchNumber,teamA:[...room.matchTeams.A],teamB:[...room.matchTeams.B],playerNames:{...(room.matchPlayerNames??playerNames(room))},roundNumber:room.coordinator.roundNumber,confirmedRoundPackages,pendingRoundPackage:pendingEntry?.package??null,spectators:room.spectators.size,serverSequence:nextSequence()});
    broadcastRoom(room,{kind:'spectator_presence',roomId:room.id,spectators:room.spectators.size,serverSequence:nextSequence()});broadcastRooms();
  }
  ...
  wss.on('connection',ws=>{
    ws._playerId=randId(12);ws._roomId=null;ws._side=null;ws._role=null;ws._displayName='Player';ws._lastChatAt=0;ws.isAlive=true;send(ws,{kind:'hello_ack',playerId:ws._playerId,serverSequence:nextSequence()});ws.on('pong',()=>{ws.isAlive=true;});
    ws.on('message',async raw=>{let msg;try{msg=JSON.parse(String(raw));}catch{return send(ws,{kind:'error',code:'BAD_JSON',serverSequence:nextSequence()});}try{
      ...
      if(msg.kind==='spectate_room'){ws._displayName=normalizePlayerName(msg.playerName??ws._displayName);const room=rooms.get(String(msg.id||''));if(!room)throw new Error('ROOM_NOT_FOUND');joinSpectator(ws,room);return;}
      ...
      if(msg.kind==='chat_message'){
        ...
        broadcastRoom(room,{kind:'chat_message',roomId:room.id,senderId:ws._playerId,role:ws._role??'PLAYER',side:ws._side,name:normalizePlayerName(ws._displayName),text,sentAt:now,serverSequence:nextSequence()});return;
      }
      if(ws._role==='SPECTATOR')throw new Error('SPECTATOR_READ_ONLY');
      ...
      if(msg.kind==='round_declarations'){if(room.matchCompleteReports.size>0)throw new Error('MATCH_COMPLETION_IN_PROGRESS');if(room.drawProposal){room.drawProposal=null;broadcastRoom(room,{kind:'draw_declined',roomId:room.id,declinedBy:null,reason:'ROUND_LOCKED',serverSequence:nextSequence()});}const ack=c.submitDeclarations(ws._side,msg.declarations,{lockedAtServerSequence:nextSequence()});send(ws,{kind:'round_declarations_locked',side:ws._side,roundNumber:c.roundNumber,waitingForOpponent:ack.waitingForOpponent,serverSequence:nextSequence()});if(c.canReleaseRound()){const pkg=c.releaseRoundPackage({deadlineMetadata:msg.deadlineMetadata??null});room.roundHistory.push({roundNumber:pkg.roundNumber,package:pkg,confirmed:false});broadcastRoom(room,{kind:'round_package',package:pkg,serverSequence:nextSequence()});}return;}
      if(msg.kind==='round_digest'){const result=c.submitDigest(ws._side,msg.digest);if(result.kind==='round_confirmed'){room.roundReadySides.clear();room.matchCompleteReports.clear();const entry=room.roundHistory.find(x=>x.roundNumber===c.roundNumber);if(entry)entry.confirmed=true;broadcastRoom(room,{...result,serverSequence:nextSequence()});}else if(result.kind==='round_desync'){broadcastRoom(room,{...result,serverSequence:nextSequence()});c.halt('DESYNC');}else send(ws,{kind:'round_digest_received',roundNumber:c.roundNumber,waitingForOpponent:true,serverSequence:nextSequence()});return;}
  ...
  const PORT=Number(process.env.PORT||3000);server.listen(PORT,()=>{const actual=server.address()?.port??PORT;console.log(`ROS 2.0 Stage 25AG coordinator listening on ${actual}`);console.log(`[RANKED] ladder file: ${ladderFile}`);console.log(`[RANKED] persistence: ${JSON.stringify(rankedPersistence())}`);});
