...
function emptyRecord({rating=true}={}){return rating?{wins:0,draws:0,losses:0,rating:INITIAL_RATING}:{wins:0,draws:0,losses:0};}
function normalizeRecord(value,{rating=true}={}){
  const out={wins:Math.max(0,Number(value?.wins)||0),draws:Math.max(0,Number(value?.draws)||0),losses:Math.max(0,Number(value?.losses)||0)};
  if(rating)out.rating=Number.isFinite(Number(value?.rating))?Math.max(0,Math.round(Number(value.rating))):INITIAL_RATING;
  return out;
}
function games(record){return (record?.wins??0)+(record?.draws??0)+(record?.losses??0);}
...
function eloPair(ratingA,ratingB,winnerSide){
  ...
  const scoreA=winnerSide==='DRAW'?0.5:(winnerSide==='A'?1:0),scoreB=winnerSide==='DRAW'?0.5:1-scoreA;
  return {
...
class RankedLadder {
  ...
  applyData(parsed,{saveMigration=true}={}){
    ...
    if(oldVersion<2)this.rebuildRatingsFromHistory();
    if(oldVersion<3&&saveMigration)this.save();
    return true;
  }
  load(){
    try{
      if(!fs.existsSync(this.filePath))return;
      this.applyData(JSON.parse(fs.readFileSync(this.filePath,'utf8')));
    }catch(err){console.warn(`[RANKED] Failed to load ladder ${this.filePath}: ${err.message}`);}
  }
  async hydrateFromRemote(store){
    if(!store)return {configured:false,loaded:false};
  ...
  ensurePlayer(name){
    ...
    if(!p){p={name:displayName,wins:0,draws:0,losses:0,formats:Object.fromEntries(FORMATS.map(f=>[f,emptyRecord()])),lastPlayedAt:null};this.data.players[key]=p;}
    p.name=displayName;p.wins=Math.max(0,Number(p.wins)||0);p.draws=Math.max(0,Number(p.draws)||0);p.losses=Math.max(0,Number(p.losses)||0);
  ...
  ensureChampion(name){
    ...
    if(!c){c={name:displayName,wins:0,draws:0,losses:0,formats:Object.fromEntries(FORMATS.map(f=>[f,emptyRecord({rating:false})])),lastPlayedAt:null};this.data.champions[key]=c;}
    c.name=displayName;c.wins=Math.max(0,Number(c.wins)||0);c.draws=Math.max(0,Number(c.draws)||0);c.losses=Math.max(0,Number(c.losses)||0);
  ...
  rebuildRatingsFromHistory(){
    ...
    const history=Object.entries(this.data.recordedMatches)
      ...
      .filter(m=>FORMATS.includes(m.format)&&(m.winnerSide==='A'||m.winnerSide==='B'||m.winnerSide==='DRAW')&&m.playerA&&m.playerB)
  ...
  recordMatch({matchId,format,playerA,playerB,winnerSide,teamA,teamB}){
    ...
    if(winnerSide!=='A'&&winnerSide!=='B'&&winnerSide!=='DRAW')throw new Error('INVALID_RANKED_WINNER');
    ...
    const aFormat=a.formats[format],bFormat=b.formats[format],before={A:aFormat.rating,B:bFormat.rating},after=eloPair(before.A,before.B,winnerSide),now=new Date().toISOString();
    if(winnerSide==='DRAW'){
      a.draws+=1;b.draws+=1;aFormat.draws+=1;bFormat.draws+=1;
    }else{
    ...
    aFormat.rating=after.A;bFormat.rating=after.B;a.lastPlayedAt=now;b.lastPlayedAt=now;
    const updateChampion=(champion,side)=>{const c=this.ensureChampion(champion);if(winnerSide==='DRAW'){c.draws+=1;c.formats[format].draws+=1;}else{const won=side===winnerSide;c[won?'wins':'losses']+=1;c.formats[format][won?'wins':'losses']+=1;}c.lastPlayedAt=now;};
  ...
  snapshot(){
    const players=Object.values(this.data.players).map(raw=>{
      const p=this.ensurePlayer(raw?.name),wins=p.wins,draws=p.draws,losses=p.losses,total=games({wins,draws,losses}),formats={};
      for(const f of FORMATS){const r=normalizeRecord(p.formats?.[f]);formats[f]={...r,games:games(r),winPct:winPct(r),rank:null};}
      return {name:cleanName(p.name),wins,draws,losses,games:total,winPct:winPct({wins,draws,losses}),formats,lastPlayedAt:p.lastPlayedAt??null,rank:null};
    });
    players.sort((a,b)=>b.wins-a.wins||b.draws-a.draws||a.losses-b.losses||b.games-a.games||a.name.localeCompare(b.name));players.forEach((p,i)=>p.rank=i+1);
    for(const f of FORMATS){
      const ranked=players.filter(p=>p.formats[f].games>0).sort((a,b)=>b.formats[f].rating-a.formats[f].rating||b.formats[f].wins-a.formats[f].wins||b.formats[f].draws-a.formats[f].draws||a.formats[f].losses-b.formats[f].losses||a.name.localeCompare(b.name));
      ranked.forEach((p,i)=>p.formats[f].rank=i+1);
    }
    const champions=Object.values(this.data.champions).map(raw=>{
      const c=this.ensureChampion(raw?.name),wins=c.wins,draws=c.draws,losses=c.losses,total=games({wins,draws,losses}),formats={};
      ...
      return {name:cleanChampion(c.name),wins,draws,losses,games:total,winPct:winPct({wins,draws,losses}),formats,lastPlayedAt:c.lastPlayedAt??null,rank:null};
    });
